 ...
 * @LastEditTime: 2021-08-14 18:51:46
        ...
        <div class="con">
          ...
          <div class="recommend" ref="before">
            ...
            <div class="recommendCon df col df-ai-fs">
              <span
                href
                v-for="(beforeItem, index) in beforeData"
                :key="index"
                @click="getInfo(beforeItem.article_id)"
              >《{{ beforeItem.title }}》</span>
            </div>
          </div>
        </div>
        <div class="line">
      ...
      axios
        ...
        .then((res) => {
          res.data.data.data.forEach((item,index) => {
            if(item.article_id == _id){
              window.localStorage.setItem("currtData",JSON.stringify(item));
               for(let i= index+1; i<index+5;i++){
                 this.beforeData.push(res.data.data.data[i]);
                }
    ...
    getCommentInfo() {
      setTimeout(() => {
        axios
          ...
          .then((res) => {
            // console.log(typeof(this.getCurrtData));
            // console.log(this.getCurrtData.article_id);
            this.commentData = res.data.data.data;
            if(this.commentData!=[]){
              for(let i=0; i<this.commentData.length; i++){
                let item = this.commentData[i];
                // console.log(typeof( eval('('+item.comment_info+')')));
